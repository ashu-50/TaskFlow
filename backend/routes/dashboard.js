  ...
  try {
    ...
    if (!isAdmin) {
      const [memberships, ownedProjects] = await Promise.all([
        ...
        Project.findAll({ where: { ownerId: userId }, attributes: ['id'] }),
      ]);
      ...
      projectWhere = { id: { [Op.in]: accessibleIds } };
      taskWhere = { projectId: { [Op.in]: accessibleIds } };
    }
    ...
    const [
      ...
      Task.findAll({
        where: {
          ...taskWhere,
          '$Task.dueDate$': { [Op.between]: [now, next7Days] },
          status: { [Op.ne]: 'done' },
        },
        include: [
          { model: User, as: 'assignee', attributes: ['id', 'name'] },
      ...
      // ── TASKS PER USER
      Task.findAll({
        where: { ...taskWhere, assigneeId: { [Op.ne]: null } },
        attributes: [
          'assigneeId',
          [fn('COUNT', col('Task.id')), 'count'],
        ],
      ...
      Task.count({
        where: {
          ...taskWhere,
          status: 'done',
          '$Task.completedAt$': { [Op.gte]: last30Days },
        },
      }),

      isAdmin ? User.count() : Promise.resolve(null),
    ]);
    ...
    tasksByStatus.forEach(t => {
      if (statusBreakdown[t.status] !== undefined) {
        statusBreakdown[t.status] = Number(t.count);
      }
    });
    ...
    tasksByPriority.forEach(t => {
      if (priorityBreakdown[t.priority] !== undefined) {
        priorityBreakdown[t.priority] = Number(t.count);
      }
    });
    ...
    const completionRate =
      totalTasks > 0
        ? Math.round((completedLast30 / totalTasks) * 100)
        : 0;
    ...
    res.json({
      ...
      dashboard: {
        ...
        taskStatus: statusBreakdown,
        taskPriority: priorityBreakdown,
        tasksPerUser,
        overdueTasks,
        myTasks,
        upcomingDeadlines,
